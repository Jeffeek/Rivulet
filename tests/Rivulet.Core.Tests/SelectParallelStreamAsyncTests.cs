...
public sealed class SelectParallelStreamAsyncTests
{
    ...
    public async Task SelectParallelStreamAsync_EmptyCollection_ReturnsEmptyStream()
    {
        ...
        var results = await source.SelectParallelStreamAsync(static (x, _) => new ValueTask<int>(x * 2)).ToListAsync(TestContext.Current.CancellationToken);
    ...
    public async Task SelectParallelStreamAsync_StreamsResultsAsTheyComplete()
    {
        ...
        foreach (var expected in new[] { 1, 2, 3, 4, 5 }) results.ShouldContain(expected);
    ...
    public async Task SelectParallelStreamAsync_CustomChannelCapacity_HandlesBackpressure()
    {
        ...
        var options = new ParallelOptionsRivulet { ChannelCapacity = 5, MaxDegreeOfParallelism = 2 };
    ...
    public async Task SelectParallelStreamAsync_LargeCollection_StreamsCorrectly()
    {
        ...
        var count = await source.SelectParallelStreamAsync(static (x, _) => new ValueTask<int>(x * 2)).CountAsync(TestContext.Current.CancellationToken);
