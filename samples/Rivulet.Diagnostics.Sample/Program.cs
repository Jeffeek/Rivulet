...
using (new RivuletConsoleListener())
{
    ...
    await numbers.SelectParallelAsync(static async (n, ct) =>
        {
...
await using (new RivuletStructuredLogListener(jsonFile))
{
    ...
    await data.SelectParallelAsync(static async (d, ct) =>
        {
...
using (var prometheusExporter = new PrometheusExporter())
{
    ...
    await workItems.SelectParallelAsync(static async (item, ct) =>
        {
...
{
    var healthCheck = new RivuletHealthCheck(
        exporter,
        new RivuletHealthCheckOptions
        {
    ...
    await testData.SelectParallelAsync(static async (item, ct) =>
        {
...
await using (var aggregator = new MetricsAggregator(TimeSpan.FromSeconds(2)))
{
    ...
    await workload.SelectParallelAsync(static async (item, ct) =>
        {
            await Task.Delay(Random.Shared.Next(5, 30), ct);
            return item % 12 == 0 ? throw new InvalidOperationException("Error") : item;
        },
        new ParallelOptionsRivulet
        {
...
foreach (var metric in aggregatedMetricsList.GroupBy(static m => m.Name).Select(static g => g.Last()))
{
