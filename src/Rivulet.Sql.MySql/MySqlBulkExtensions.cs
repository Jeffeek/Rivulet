...
public static class MySqlBulkExtensions
{
    ...
    /// </remarks>
    public static async Task BulkInsertUsingMySqlBulkLoaderAsync(
                    ...
                    await using (connection)
                    {
                        ...
                        catch (Exception ex)
                        {
                            throw new InvalidOperationException(
                                $"Failed to bulk load batch of {batch.Length} rows to table '{tableName}'", ex);
                        }
                    }
                }
                finally
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Task representing the async operation</returns>
    public static async Task BulkInsertFromFilesUsingMySqlBulkLoaderAsync(
        this IEnumerable<string> source,
        Func<MySqlConnection> connectionFactory,
        string tableName,
        string[] columnNames,
        ...
        await source
            .ForEachParallelAsync(async (filePath, ct) =>
            {
                ...
                await using (connection)
                {
                    ...
                    {
                        throw new InvalidOperationException(
                            $"Failed to bulk load file '{filePath}' to table '{tableName}'", ex);
                    }
                }
            }, options, cancellationToken)
            .ConfigureAwait(false);
